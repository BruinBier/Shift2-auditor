...
const QUICK_FINDINGS_DATA = [
  {
    ...
    criterionCode: '1.1.1',
    impact: 'klein' as any,
    responsibility: 'redacteur' as any
  },
  {
...
// POST /api/seed-quick-findings - Seed quick findings data
export async function POST() {
  try {
    // Check if quick findings already exist
    const existingCount = await prisma.quickFinding.count();

    if (existingCount > 0) {
      return NextResponse.json({
        message: 'Quick findings already seeded',
        count: existingCount
      });
    }

    // Seed the quick findings
    const results = await prisma.quickFinding.createMany({
      data: QUICK_FINDINGS_DATA.map(qf => ({
        ...qf,
        impact: qf.impact as any,
        responsibility: qf.responsibility as any
      }))
    });

    return NextResponse.json({
      message: 'Quick findings seeded successfully',
      count: results.count
    });
  } catch (error) {
    console.error('Error seeding quick findings:', error);
    return NextResponse.json(
      { error: 'Failed to seed quick findings' },
      { status: 500 }
    );
  }
}