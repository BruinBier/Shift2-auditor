...
- LIJSTITEM MET LINK EN TOELICHTING: een <li> dat naast de link ook een <br> en enkele zinnen toelichting bevat, is CORRECT. Geen bevinding en geen opmerking, ook niet als best practice. De toelichting hoort inhoudelijk bij die link, dus in hetzelfde lijstitem staat ze precies goed: het bulletje staat vóór de link en de toelichting staat ingesprongen onder datzelfde bulletje, dus code en weergave zeggen hetzelfde. Een lijstitem mag meer bevatten dan alleen een link, en een <br> binnen een lijstitem is geen structuurfout. Twee argumenten die NIET gelden: "visueel lijkt de toelichting een apart tekstblok" (dat is niet zo, zie hierboven) en "dit item wijkt af van de andere items in de lijst" (een consistentie-observatie, geen toegankelijkheidsprobleem). Voorbeeld: BEV-03 B010 (duurzaam.beverwijk.nl/subsidies-en-leningen) is om deze reden op 2026-07-30 verwijderd. Vergelijk de regel hierboven over een lijst met één item: dát is wel een opmerking.
...
- DE TAGBOOM IS DE HELFT VAN HET ANTWOORD. Render de pagina en leg de twee naast elkaar. Een opsomming die in de tagstructuur vlak is, kan op het scherm twee niveaus hebben: de inspringing zit dan in de opmaak en niet in de structuur. Wie het document laat voorlezen hoort dan gelijkwaardige punten waar er een uitwerking van een punt erboven staat. Dat is een AFKEURING (matig, redacteur).
  Dit geldt ook andersom en voor de andere deelgebieden: een gebied als "Geneste lijsten" is pas nagelopen als je hebt gekeken of er visueel nesting is, niet alleen of er in de tags een tweede niveau staat. Een leeg resultaat in de tagboom en een opsomming die werkelijk vlak is, zien er daar hetzelfde uit.
  Vastgesteld door Frits op 2026-09-18 bij ZOET-01, Bijlage 2 pagina 7: vier streepjes onder "Werkwijze", waarvan de laatste twee ingesprongen staan onder het tweede punt, terwijl alle vier in de tagboom op hetzelfde niveau staan.
...
- GEDEELTELIJK getagde PDF: een document met tags waarvan niet alle pagina's meedoen, is een AFKEURING onder 1.3.1 (impact serieus, verantwoordelijkheid redacteur). Stel het vast door de pagina's met `/StructParents` te tellen en naast het totaal te leggen; loopt dat uiteen, dan MOET het gemeld worden. Zonder die melding valt het ongetagde deel stilzwijgend buiten het onderzoek: het document volgt de route "wel tags" en de beoordeling gaat over de kwaliteit van tags die op een deel van de pagina's niet bestaan. Schrijf het als één bevinding over één document: het document is getagd, maar niet op alle pagina's, met de paginanummers erbij. Niet "er zitten twee documenten in" en niet "de bijlage is ongetagd" — hoe het zo gekomen is, is een verklaring en geen bevinding. Het advies wijst naar het ongetagde deel en niet naar het geheel: het hoofddocument opnieuw exporteren lost niets op, want het ongetagde deel komt er daarna net zo weer achter. Komt dat deel van een externe partij, dan hoort de vraag om een toegankelijke versie daar te liggen. Voorbeeld: ZOET-01 Bijlage 2, getagd tot en met pagina 99 van de 166. Vastgesteld door Frits op 2026-09-17.
- Niet-getagde PDF: de ontbrekende tagstructuur afkeuren onder 1.3.1 (dit is de wortel-oorzaak). Dat is een AFKEURING, status open, impact SERIEUS, verantwoordelijkheid REDACTEUR: dat de structuur ontbreekt is zonder tags wél vast te stellen, dus het is een echte fout. Je ziet het in PAC aan een lege tagboom (geen structuurelementen, geen roltoewijzingen) en aan "Tags: 0" bovenaan zijn scherm, maar je stelt het ZELF vast: volg `/StructTreeRoot` naar zijn object en kijk of daar `/K` in staat (de kinderen van de boom). Geen `/K` betekent ongetagd, ook als de catalog een `/StructTreeRoot` heeft; `/MarkInfo /Marked` doet niet mee, in welke stand dan ook. Tel niet de losse `/StructElem`-objecten: die zitten in gecomprimeerde objectstromen en geven nul voor elk document. Schrijf er NIET bij dat het document zichzelf als getagd aanmerkt: dat is de PAC-melding "Markering voor getagde documenten" en die negeren we (`/MarkInfo /Marked` is een administratieve vlag, geen obstakel voor wie het document gebruikt). Vastgesteld door Frits op 2026-09-17. Maak het als PAAR met de 1.1.1-opmerking voor hetzelfde document (status resolved, impact en verantwoordelijkheid leeg; zie Shift2_Regels_SC_1_1_1.md): twee bevindingen op dezelfde PDF, één afkeuring en één opmerking. De 1.1.1 is een opmerking omdat je zonder tags niet kunt vaststellen wélke tekstalternatieven ontbreken; de 1.3.1 is een afkeuring omdat het ontbreken van de structuur zelf wel vaststaat. Voorbeeld: BEV (Duinrust), B030 is de 1.1.1-opmerking en B031 de 1.3.1-afkeuring; zo ook Heerlen-01 B011, BEL-03 B018 en BEL-04 B030. Geen tag-jargon in de tekst: schrijf "als afbeelding gemarkeerd", niet "als Figure". Criteria die je alleen kunt beoordelen wanneer er tags zijn, keur je NIET apart af als gevolg van diezelfde oorzaak. De scheidslijn is NIET "het document is niet getagd, dus niets is te beoordelen". De vraag is
...
`npm run cli -- get-pdfcontrast <pdf-url>`; dat werkt onafhankelijk van de tagstructuur, dus
PAC hoeft er niet aan te pas te komen. Staat de tekst op een foto of een verloop, dan is de
uitkomst een band en geen enkel getal; zie `Shift2_Regels_SC_1_4_3.md`.
