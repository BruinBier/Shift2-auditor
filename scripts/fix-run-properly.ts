...
async function fixTemplate() {
  ...
  const current = xmlContent.substring(runEnd + '</w:r>'.length, lineBreak + '<w:br/>'.length);
  console.log('\n=== CURRENT ===');
  console.log(current);
  console.log('===============\n');

  // Remove everything between the end of {reportIntroHeader} run and just before the line break
  // Then add the line break in its own properly formatted run
  xmlContent = xmlContent.substring(0, runEnd + '</w:r>'.length) +
               '<w:r><w:br/></w:r>' +
               xmlContent.substring(lineBreak + '<w:br/>'.length);

  console.log('✓ Fixed run structure');
