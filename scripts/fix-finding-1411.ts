...
async function main() {
  ...
  console.log('Finding bevinding for SC 1.4.11...');

  // First, find the WCAG criterion 1.4.11
  const criterion = await prisma.wCAGCriterion.findFirst({
    where: { code: '1.4.11' }
  });

  if (!criterion) {
    console.log('WCAG Criterion 1.4.11 not found');
    return;
  }
  ...
  console.log(`\nFound ${findings.length} finding(s) for SC 1.4.11`);
