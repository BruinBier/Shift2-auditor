...
async function main() {
  console.log('Start seeding quick findings...');

  // Delete all existing quick findings
  const deleted = await prisma.quickFinding.deleteMany();
  console.log(`Deleted ${deleted.count} existing quick findings.`);

  // Seed all quick findings
  let createdCount = 0;
  for (const finding of QUICK_FINDINGS) {
    try {
      ...
      console.error(`Failed to create: ${finding.title}`, error);
    }
  }

  console.log(`\nSeeding completed! Created ${createdCount} quick findings.`);
}
